...
export default function IssueHubPage() {
  ...
  const handleSearchAndFilter = async () => {
    try {
      ...
      if (searchQuery.trim()) {
        filteredPosts = await searchQuestions(
          searchQuery.trim(),
  ...
  const handleAnswerSubmit = async (questionId: string, content: string) => {
    try {
      ...
      if (result.success) {
        ...
        return {
          success: false,
          message: result.message || "Failed to post answer",
        };
      }
    } catch (error) {
                ...
                {loading ? (
                  ...
                  Array(5)
                    ...
                    .map((_, i) => (
                      <Card
                        key={i}
                        className="p-6 bg-card border-secondary animate-pulse"
                      >
                        ...
                        <div className="flex items-start gap-4">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-2">
                              <Badge
                                variant="outline"
                                className="border-secondary text-foreground text-[10px] h-5"
                              >
                                {post.category}
                              </Badge>
                              {post.status === "answered" && (
                                <Badge className="bg-green-500/10 text-green-500 border-none text-[10px] h-5">
                            ...
                            <div className="flex items-center justify-between">
                              <div className="flex flex-wrap gap-1.5">
                                {post.tags.map((tag, index) => (
                                  <span
                                    key={index}
                                    className="text-[10px] px-2 py-0.5 rounded-md bg-secondary text-foreground"
                                  >
                                    #{tag}
                                  </span>
                                ))}
                ...
                <div className="space-y-4">
                  {trendingPosts.map((post) => (
                    <div
                      ...
                      onClick={() => setSelectedQuestionId(post.id)}
                    >
                      <h4 className="text-sm font-medium text-foreground group-hover:text-primary transition-colors line-clamp-1 mb-1">
                        {post.title}
                      </h4>
